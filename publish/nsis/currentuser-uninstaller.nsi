...
LangString UNCONFIRM_SUBTITLE ${LANG_ENGLISH} "Remove WinCraft from your computer."
...
LangString UNINSTALL_FINISH_SUBTITLE ${LANG_ENGLISH} "WinCraft was successfully removed."

LangString UNCONFIRM_SUBTITLE ${LANG_SIMPCHINESE} "从你的计算机中移除 WinCraft。"
...
LangString UNINSTALL_FINISH_SUBTITLE ${LANG_SIMPCHINESE} "WinCraft 已成功移除。"

BrandingText "WinCraft"

VIProductVersion "${Version}.0"
VIAddVersionKey "CompanyName"      "YeahOSS"
VIAddVersionKey "FileDescription"  "WinCraft Current-User Uninstaller"
...
Function .onInit
  ...
  ${If} $0 != ""
    StrCpy $INSTDIR "$0"
  ${ElseIf} $INSTDIR == "$LOCALAPPDATA\WinCraft"
    StrCpy $INSTDIR "$EXEDIR"
  ${EndIf}

  ; Upgrade mode is launched by the installer with ExecWait, so keep this
  ; process synchronous instead of handing work to a temp copy.
  ${If} $UpgradeMode == "1"
    StrCpy $KeepLogs "1"
    StrCpy $KeepDumps "1"
    Return
  ...
  ${If} $FromTemp != "1"
    CopyFiles /SILENT "$EXEPATH" "$TEMP\WinCraft-CurrentUser-Uninstall.exe"
    ${If} ${Silent}
      Exec '"$TEMP\WinCraft-CurrentUser-Uninstall.exe" /fromtemp /S /D="$INSTDIR"'
    ${Else}
      Exec '"$TEMP\WinCraft-CurrentUser-Uninstall.exe" /fromtemp /D="$INSTDIR"'
    ${EndIf}
    SetErrorLevel 0
    Quit
  ${EndIf}
