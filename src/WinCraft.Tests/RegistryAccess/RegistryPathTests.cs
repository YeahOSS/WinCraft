...
namespace WinCraft.Tests.RegistryAccess
{
    ...
    internal sealed class RegistryPathTests
    {
        ...
        {
            var path = RegistryPath.FromClassesRoot("*").Combine("Shell").Combine("open");

            Assert.That(path.ToString(), Is.EqualTo(@"HKEY_CLASSES_ROOT\*\Shell\open"));
            Assert.That(path.Parent.ToString(), Is.EqualTo(@"HKEY_CLASSES_ROOT\*\Shell"));
            Assert.That(path.Name, Is.EqualTo("open"));
        }
    }
}
